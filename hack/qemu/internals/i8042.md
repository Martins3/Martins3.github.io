...
ps2_queue_noirq 的调用者可以从 guest ，也可以从 Host 到来:

## guest 如何接受数据
从 ./drivers/input/serio/i8042-x86ia64io.h:i8042_platform_init 看，这两个端口就是固定好了的，如果不考虑 pnp 
...
使用 strace bash 来分析 ptrace 到底在做什么?
```c
/*
...
## /dev/tty
标准输出最后成为了 tty ? 之前的 evdev_open 是在搞什么东西?

- [ ] 总结一下，总之，使用 qemu 比较清楚的来分析了 char_dev, tty, /dev/input/event 之类的东西，而且也分析了 interupt 和 select 等操作, i8042 的中断注册两个号码。
  - 观测到了键盘中断在内核的处理过程
  - [ ] tty 和标准输出怎么搞到一起的暂时不知道
