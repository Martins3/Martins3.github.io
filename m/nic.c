...
static void snull_setup_pool(struct net_device *dev)
{
	...
	for (i = 0; i < pool_size; i++) {
		...
		if (pkt == NULL) {
			printk(KERN_NOTICE
			       "Ran out of memory allocating packet pool\n");
			return;
		}
		pkt->dev = dev;
		pkt->next = priv->ppool;
