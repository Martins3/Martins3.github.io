...
function cmd_hmp() {
	# 类似这种资源都是当有两个都是都是需要再次选择的
	# 但是需要再次选择的情况并不多
	if [[ $live_qemu_count == 2 ]]; then
		items=(source target)
		set -x
		hmp_which=$(printf "%s\n" "${items[@]}" | fzf)
		source=$(cat "$vm_dir"/migrate_source)
		get_another_qemu "$source"
		target=$another_qemu

		if [[ $hmp_which == source ]]; then
			which_qemu=$source
		else
			which_qemu=$target
		fi
...
migrate_host=""
function choose_migrate_host() {
	if ! check_option nbd; then
	...
	local host
	host=$(printf "%s\n" "${fruits[@]}" | fzf)
	readarray -t local < <(ip -4 addr show | grep inet | awk '{print $2}' | cut -d'/' -f1)
	for i in "${local[@]}"; do
		if [[ $i == host ]]; then
			error "migrate to local ?"
		fi
	done
	migrate_host=$host
}
...
function do_migration() {
	...
	case $testcase in
		...
		background-snapshot)
			...
			#
			# 想法也非常的简单，我猜测是，立刻保存 vmstate ，然后对于内存进行 write protect
			# 然后开启一个 background 来把内存全部都写入盘中。如果一个 page 还没写回，遇到了 write page fault ，
			# 那么就 stall 住，让这个页面立刻写回。写会之后，页面保护接触。
			#
			# 这个功能需要开启 userfault ，注意要解决权限问题
			...
			;;
		vcpu)
			# info migrate_parameters 中发现似乎可以限制 vcpu 的执行
			# x-vcpu-dirty-limit-period: 1000 ms
			# vcpu-dirty-limit: 1 MB/s
			#
			# 那么，我问下，这个 vCPU 是如何限制的，
			# 会导致虚拟机中感觉到 vCPU 跳变吗，remote tlb 没有响应
			;;
		*)
			error "unknown migrate setting"
			;;
	esac
	...
	# 什么东西叫做 return-path 啊
	# Error: Capability 'switchover-ack' requires capability 'return-path'

	cmds+=(
		# max-bandwidth 的单位是 MB/s
	...
	for cmd in "${cmds[@]}"; do
		echo "$cmd" \
			| socat - unix-connect:"$vm_dir/$source/hmp"
	done

	while true; do
		result=$(echo ' { "execute": "qmp_capabilities" } { "execute": "query-status" }' \
			| socat - unix-connect:"$vm_dir/$source"/qmp | jq -s '.[2]' \
			| jq -r '.return.status')
		# hmp "info migrate -a"
...
function migrate_to_remote() {
	# 记得到时候的虚拟机中需要使用类似这个命令的东西:
	# "migrate-incoming -incoming tcp:0:4000"
	log "似乎还需要一些工作，但不多"
	# 这个 ping 是不会终结的
	# ping "$migrate_host"
	"$PROGDIR"/nbd/c.py --operation migrate \
		--host "$migrate_host" \
		--dir "$vm_dir"
	# 可以添加一个新命令直接 launch ，但是意义不大，除非有批量的任务的时候
	exit 0
}

function migrate_to_local() {
	# 本地热迁移的场景
	gum confirm "rk -a"
	source=$(cat "$vm_dir"/migrate_source)
	target=$(cat "$vm_dir"/migrate_target)
	# echo "migrate -d tcp:$migrate_host:4000" \
	# 	| socat - unix-connect:"$vm_dir/$source/hmp"

	local option="zstd"
	# local option="mapped-ram"
	# Error: Migration requires seekable transport (e.g. file)
	case "$option" in
		zstd)
			local cmds=(
				"migrate_set_capability multifd on"
	...
	for cmd in "${cmds[@]}"; do
		echo "$cmd" \
			| socat - unix-connect:"$vm_dir/$target/hmp"
	done

	hmp_migrate="migrate -d unix:$vm_dir/migrate.sock"
	do_migration "$hmp_migrate" "$option"

	echo "socat -,echo=0,icanon=0 unix-connect:$vm_dir/$target/hmp"
	socat -,echo=0,icanon=0 unix-connect:"$vm_dir/$source/hmp"
}
...
function cmd_migrate_cpr() {
	...
	qmp_to_qemu "$source" '"execute":"migrate-set-parameters", "arguments":{"mode":"cpr-transfer"}'
	qmp_to_qemu "$source" '"execute": "migrate", "arguments": { "channels": [ {"channel-type": "main", "addr": { "transport": "socket", "type": "inet", "host": "0", "port": "44444" }}, {"channel-type": "cpr", "addr": { "transport": "socket", "type": "unix", "path": "/tmp/cpr.sock" }}]}'
}
