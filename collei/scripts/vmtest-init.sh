...
# COPY FROM : https://github.com/danobi/vmtest
#
...
# 不要给 qga_logs 添加双引号，该命令会变为 '--logfile /dev/kmsg' ，导致命令执行失败
# shellcheck disable=2086
qemu-ga --method=virtio-serial --path="$vport" $qga_logs -d || true
...
# TODO 补充一点，如果这里直接用 /bin/bash 的话，是没有办法使用 ctrl-c 的
