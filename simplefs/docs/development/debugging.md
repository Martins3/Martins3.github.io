...
目录 extent 固定覆盖 8 个块，但目录最大条目数曾错误复用普通文件单 extent 的 1024 块上限。结果满目录测试会继续尝试创建数百万条目，而真实目录格式最多只能容纳 40,920 条。修正后使用独立的 `SIMPLEFS_DIR_BLOCKS_PER_EXTENT`，达到格式上限立即返回 `EMLINK`。

这还不等于 `generic/558` 会很快结束。VFS 在 create 前必须 lookup 名字；目录达到上限后，对每个新名字仍要线性扫描 40,920 个现有项才能证明它不存在。上游用例把所有 worker 放在同一目录，剩余数十万次失败 lookup 因此退化成 O(n²)。runner 的兼容补丁把 worker 分散到独立子目录，仍然并发耗尽全部 inode，但不再要求单目录突破磁盘格式上限。能力边界、错误返回和测试工作负载必须分别核对，任何一个都不能代替另外两个。
