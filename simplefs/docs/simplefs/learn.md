...
这里不能把所有 ioend 无条件直接交给通用 submit。预分配得到的
unwritten extent 必须在数据 bio 成功完成后才转换为 written；当前实现为
unwritten ioend 安装完成回调，在 workqueue 中转换实际 I/O 范围，然后调用
`iomap_finish_ioends()`。普通 written ioend 仍直接使用
`iomap_ioend_writeback_submit()`。

另一个容易混淆的边界是 `IOMAP_F_NEW`。extent 插入后可能和旧 extent 合并，
但 iomap 的返回长度只能覆盖本次真正新分配的块，不能返回合并后的全部长度；
否则 iomap 对新块的零化会破坏相邻旧数据。
...
这份文档保留原 README 中偏个人视角的过程记录；当前技术入口见 [README.md](README.md)。

1. 起点

SimpleFS 不是从零开始的文件系统。选择它是因为文件系统集中体现了 Linux 内核中最难的同步、缓存和持久化问题，也因为早期学习文件系统时积累的很多疑问一直没有得到实验验证。

最初阶段主要靠人工完成内核接口兼容和部分 iomap 替换。后续尝试过多个 AI 编程工具，逐步把工作方式变成：先搭建可循环、可恢复的测试环境，再让工具围绕可重复失败持续定位和验证。

2. 工具使用阶段

- 早期工具生成过很长的 xfstests 驱动，后来将其缩减并持续修正环境和结果分类。
- 中期开始依赖 xfstests，而不是只依赖小型自测；很多真实语义错误只有 fallocate、mmap、DIO 和随机 fsx 的组合才能暴露。
- 后期重点从“生成代码”转为“生成可审查证据”：固定 seed、保存 workload、定位第一个错误操作、对照内核状态和重复回归。

3. 反思

项目最重要的经验不是工具能写多少代码，而是环境和测试是否能形成闭环。文件系统改动尤其需要自证明：每个修复都应说明此前违反了什么不变量、怎样稳定复现、为什么新时序或所有权关系正确，以及回归覆盖到了哪里。
