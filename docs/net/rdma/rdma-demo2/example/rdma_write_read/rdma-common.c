...
void on_completion(struct ibv_wc *wc)
{
	...
	if (wc->opcode & IBV_WC_RECV) {
		...
		if (conn->recv_msg->type == MSG_MR) {
			...
			if (conn->send_state ==
			    SS_INIT) /* received peer's MR before sending ours, so send ours back */
				send_mr(conn);
		}
	...
	if (conn->send_state == SS_MR_SENT && conn->recv_state == RS_MR_RECV) {
		...
		wr.opcode = (s_mode == M_WRITE) ? IBV_WR_RDMA_WRITE :
						  IBV_WR_RDMA_READ;
		wr.sg_list = &sge;
		wr.num_sge = 1;
...
void register_memory(struct connection *conn)
{
	...
	TEST_Z(conn->recv_mr = ibv_reg_mr(
		       ...
		       IBV_ACCESS_LOCAL_WRITE |
			       ((s_mode == M_WRITE) ? IBV_ACCESS_REMOTE_WRITE :
						      IBV_ACCESS_REMOTE_READ)));

	TEST_Z(conn->rdma_local_mr =
		       ibv_reg_mr(s_ctx->pd, conn->rdma_local_region,
