...
* [SMM](#smm)
  ...
  * [SMM 的使用](#smm-的使用)
* [问题](#问题)
* [piix4 的主板定义](#piix4-的主板定义)
* [firecracker 的 bios 处理办法](#firecracker-的-bios-处理办法)
* [TODO](#todo)
...
seabios 和 qemu 简直无聊至极，在这里搞一堆硬编码，例如这里的
```c
void
smm_setup(void)
{
...
## firecracker 的 bios 处理办法
通过观察 br kvm_ioctls::kvm_ioctls::SET_REGS 的 backtrace ，
很容易可以找到 configure_system_for_boot ，这个再次验证我的很多想法

1. 根本就不需要所谓的 16bit -> 32bit -> 64bit mode 的转换，直接加载内核，然后一次性的可以配置好内核的位置
2. 之所以需要 bios 是为了 grub ，现在只是启动内核，所以不需要 bios 了
3. firecracker 只能启动 linux ，而且必须提供 bzImage

- firecracker::main
  - firecracker::main_exec
    - firecracker::api_server_adapter::run_with_api
      - firecracker::build_microvm_from_json
        - vmm::builder::build_and_boot_microvm
          - vmm::builder::build_microvm_for_boot
            - vmm::builder::configure_system_for_boot
              - vmm::vstate::vcpu::x86_64::KvmVcpu::configure
                - vmm::arch::x86_64::regs::setup_regs
                  - kvm_ioctls::ioctls::vcpu::VcpuFd::set_regs
                    - kvm_ioctls::kvm_ioctls::KVM_SET_REGS

firecracker 处理 SMM 的 ioport 0xb2 的方法就是直接忽视掉，就像是 /dev/null 一样

## TODO
可以看看，内核是如何探测启动盘，通过 ACPI 告知的?

disk 的第一个块放着的 MBR 是如何处理的
