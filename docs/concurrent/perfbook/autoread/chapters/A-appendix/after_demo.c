/*
 * after_demo.c: Demonstrate "time going backwards" without locking.
 *
 * Based on perfbook appendix/questions/after.tex (CodeSamples/api-pthreads/QAfter/time.c)
 *
 * This program shows that on SMP systems, a consumer can observe
 * timestamps that appear to go backwards because the producer and
 * consumer are not synchronized.
 *
 * Build: make after_demo.out
 * Run:   ./after_demo.out
 */
...
static void *consumer(void *ignored)
{
	...
	printf("consumer exited loop, collected %d items\n", i);
	if (ssc[0].iserror)
