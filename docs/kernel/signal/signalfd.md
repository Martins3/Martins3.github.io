...
也就是说，signalfd_ctx 只保存了一个 sigmask，没有任何指向创建者 task 的引用。无论这个 fd 被 dup、fork 还是通过 UDS SCM_RIGHTS
传到哪个进程，读它的时候永远看的是当前进程的 task->pending / task->signal->shared_pending。
...
简言之：signalfd 不是跨进程共享信号的事件源，它只是把当前进程的信号投递转换成 fd
可读事件。想监控哪个进程的信号，就必须在那个进程里读它自己的 signalfd。
...
signalfd 没有想象的那么好用的:
https://news.ycombinator.com/item?id=33300856
