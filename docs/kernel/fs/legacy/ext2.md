...
其实没必要看这个，最简单的还是看个 minix 的文件系统。

还是感觉有必要仔细看看，挺有趣的, 从 fs/ext2/dir.c 开始吧
- ext2_dir_operations::fsync 为什么 ext2_dir_operations::fsync 和
- ext2_set_link 实现和被调用的
...
```txt
#0  ext2_mount (fs_type=0xffffffff831cf6a0 <ext2_fs_type>, flags=0, dev_name=0xffff88810301bce0 "/dev/vdc",
    data=0x0 <fixed_percpu_data>) at fs/ext2/super.c:1476
#1  0xffffffff814ad168 in legacy_get_tree (fc=0xffff88810f688900) at fs/fs_context.c:662
#2  0xffffffff81461c16 in vfs_get_tree (fc=0xffffffff831cf6a0 <ext2_fs_type>, fc@entry=0xffff88810f688900) at fs/super.c:1771
#3  0xffffffff814918ac in do_new_mount (data=0x0 <fixed_percpu_data>, name=0xffff88810301be50 "/dev/vdc", mnt_flags=32,
    sb_flags=0, fstype=<optimized out>, path=0xffffc90000e4fef8) at fs/namespace.c:3337
#4  path_mount (dev_name=dev_name@entry=0xffff88810301be50 "/dev/vdc", path=path@entry=0xffffc90000e4fef8,
    type_page=type_page@entry=0xffff888102f9d258 "ext2", flags=<optimized out>, flags@entry=0,
    data_page=data_page@entry=0x0 <fixed_percpu_data>) at fs/namespace.c:3664
#5  0xffffffff8149219a in do_mount (data_page=0x0 <fixed_percpu_data>, flags=0, type_page=0xffff888102f9d258 "ext2",
    dir_name=0x55cda2bc9780 "/mnt", dev_name=0xffff88810301be50 "/dev/vdc") at fs/namespace.c:3677
#6  __do_sys_mount (data=<optimized out>, flags=0, type=<optimized out>, dir_name=0x55cda2bc9780 "/mnt",
    dev_name=<optimized out>) at fs/namespace.c:3886
#7  __se_sys_mount (data=<optimized out>, flags=0, type=<optimized out>, dir_name=94341686925184, dev_name=<optimized out>)
    at fs/namespace.c:3863
#8  __x64_sys_mount (regs=<optimized out>) at fs/namespace.c:3863
#9  0xffffffff82398be3 in do_syscall_x64 (nr=<optimized out>, regs=0xffffc90000e4ff58) at arch/x86/entry/common.c:51
#10 do_syscall_64 (regs=0xffffc90000e4ff58, nr=<optimized out>) at arch/x86/entry/common.c:82
#11 0xffffffff824000eb in entry_SYSCALL_64 () at arch/x86/entry/entry_64.S:120
```

看上去 mount_bdev 是一些低端 fs 的经常走到的地方

## 虚拟机中无法 mount ext2
https://unix.stackexchange.com/questions/324200/why-does-attr-l-tell-me-about-an-selinux-attribute-that-attr-g-selinux

报错原因:
```txt
[  914.558399] SELinux: (dev vdc, type ext2) has no xattr support
```
