...
2. graft_tree 在此处应该是不存在的 @todo 但是从哪里可以访问这个，以及其中 inode 在哪里，
是不是这些不需要 graft_tree 的都是没有 tree 结构的，所有都是放在 root inode 下
...
~ 🐕 took 11s
🧀  t
sudo bpftrace -e 'kprobe:mount_bdev { @[kstack(bpftrace)] = count(); }' -c "sleep 10"
Attaching 1 probe...
^C
