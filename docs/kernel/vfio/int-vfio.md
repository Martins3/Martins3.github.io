...
```txt
ioctl(dev->device_fd, VFIO_DEVICE_GET_IRQ_INFO, irq); // 获取一共有多少个中断
int irqfd = eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC); // 注册一个 eventfd
...
而对于 qemu 而言，这个 eventfd 是通过 VM_IRQFD 获取的，这个时候，当中断到来，被通知的
不再是用户态的 eventfd ，而是 irqfd_wakeup 被调用
...
1. PIN : Posted-interrupt notification event : vfio 和 virtio-blk 都可能，当触发 posted interrupts ，但是 vCPU 正好没运行，就会记录下来
