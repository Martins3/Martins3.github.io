...
// 让 Kimi 简单修改了一下，从而可以被
// 配合 echo "h" | nc -u 127.0.0.1 8877 来观察事件被触发
...
fn do_work() -> Result<()> {
    ...
    // (1) 修改1: 固定端口 8877，并打印地址
    let sink = UdpSocket::bind("127.0.0.1:8877")?;
    println!("UDP server listening on {}", sink.local_addr()?);
    let sink_fd = types::Fd(sink.as_raw_fd());

    let mut rd_buf = [0u8; 1024];  // 增大缓冲区到 1024 字节
    let rd_op = opcode::Recv::new(sink_fd, &mut rd_buf as _, rd_buf.len() as _);  // (2) 修改2: Read -> Recv
    ...
    loop {
        ...
        for r in &mut rings {
            drain_cq(&mut r.completion(), &mut rd_buf)?;  // 传入 buffer 用于打印
        }
    }
}
...
// (3) 修改3: 增加 buffer 参数用于打印收到的数据
fn drain_cq(cq: &mut CompletionQueue, buf: &mut [u8]) -> Result<()> {
    ...
    for cqe in cq.into_iter() {
        ...
        if r > 0 {
            ...
            if let Ok(s) = std::str::from_utf8(data) {
                println!("Received ({} bytes): {}", r, s.trim_end());
            }
        }
        assert!(
            r >= 0 || r == -libc::EINTR,
