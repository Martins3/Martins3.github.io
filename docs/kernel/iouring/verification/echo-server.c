 ...
 *
 * 使用方法:
 * 1. ./liburing-echo-server.out 8888
 * 2. nc localhost 8888 输入什么东西，就会回显什么东西
 *
 * cqe type = 1 (READ)
 * [+] add_socket_write ffffffffffffffffff
 * cqe type = 2 (WRITE)
 * [+] add_socket_read
 * cqe type = 3 (PROV_BUF)
 *
 * 基本原理:
 * - struct conn_info 是这个程序自定义的结构体，add_socket_write / add_socket_read / add_accept 都是提交
 * 然后让 io_uring_submit_and_wait 返回等待当时提交的结果。
 *
 * 这个例子很有意思，通过 add_accept 提交任务之后，那么 io_uring_submit_and_wait 就是自动实现了 epoll 的功能。
 *
 * */
#include <errno.h>
#include <fcntl.h>
	...
	while (1) {
		...
		io_uring_for_each_cqe(&ring, head, cqe)
		{
			...
			if (cqe->res == -ENOBUFS) {
				...
				if (sock_conn_fd >= 0) {
					add_socket_read(&ring, sock_conn_fd,
							group_id,
							MAX_MESSAGE_LEN,
							IOSQE_BUFFER_SELECT);
				}
				...
				if (cqe->res <= 0) {
					...
					add_socket_write(&ring, conn_i.fd, bid,
							 bytes_read, 0);
				}
			} else if (type == WRITE) {
				...
				add_socket_read(&ring, conn_i.fd, group_id,
						MAX_MESSAGE_LEN,
						IOSQE_BUFFER_SELECT);
			}
		}
...
{
	struct io_uring_sqe *sqe = io_uring_get_sqe(ring);
	io_uring_prep_provide_buffers(sqe, bufs[bid], MAX_MESSAGE_LEN, 1, gid,
				      bid);
