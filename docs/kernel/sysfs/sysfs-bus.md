...
- bus_register : /sys/bus 在这里创建出来一个 /sys/fs bus 驱动
...
lspci 显示在 pci 下接入了一个 ISA bridge
```plain
00:1f.0 ISA bridge: Intel Corporation Sunrise Point LPC Controller/eSPI Controller (rev 21)
```

[How to list the devices attached to an ISA bus in Linux?](https://unix.stackexchange.com/questions/397701/how-to-list-the-devices-attached-to-an-isa-bus-in-linux)

看了下，这个东西真的太老了，不用浪费时间了

## platform bus

总体来说，就是那些不是挂到 pci 的设备，而 platform 就是南桥/pch 上的那些芯片

### [Platform Devices and Drivers](https://www.kernel.org/doc/html/latest/driver-api/driver-model/platform.html)

./sysfs-bus.txt 中当时还记录了一份日志。
...
```txt
[    1.121656] ->:platform_match QEMU0002:00 == alarmtimer
[    1.131440] ->:platform_match PNP0103:00 == alarmtimer
[    1.141871] ->:platform_match regulatory == alarmtimer
[    1.150975] ->:platform_match pcspkr == alarmtimer
[    1.161217] ->:platform_match platform-framebuffer == alarmtimer
[    1.186162] ->:platform_match QEMU0002:00 == efi-framebuffer
[    1.196054] ->:platform_match PNP0103:00 == efi-framebuffer
[    1.206521] ->:platform_match regulatory == efi-framebuffer
[    1.215138] ->:platform_match pcspkr == efi-framebuffer
[    1.224812] ->:platform_match platform-framebuffer == efi-framebuffer
```

## [ ] acpi bus 和 platform_bus 的关系是什么?
platform_probe 在 [^4] 中描述是用于探测真正的物理设备是否存在，但是实际上，legacy driver model 的设备才会调用
```txt
[    0.989154] ->:platform_probe serial8250==serial8250
[    1.030243] ->:platform_probe floppy==floppy
[    1.390907] ->:platform_probe i8042==i8042
[    1.409823] ->:platform_probe alarmtimer==alarmtimer
```
...
```txt
/*
[    0.582758] -> device: 'QEMU0002:00': device_add
...
[    0.583816] -> device: 'PNP0103:00': device_add
...
```txt
/*
[    1.493913] -> device: 'regulatory.0': device_add
...
[    1.533810] -> device: 'pcspkr': device_add
...
[    1.541961] -> device: 'platform-framebuffer.0': device_add
...
## 奇怪的 rtc
```txt
[    2.308385] ->:platform_match alarmtimer == alarmtimer
...
```txt
🧀  ls -la  /sys/devices/LNXSYSTM:00/
.r--r--r-- 4.1k root  6 Jul 12:08  hid
drwxr-xr-x    - root  6 Jul 12:08  INT3420:00
drwxr-xr-x    - root  6 Jul 12:08  LNXPOWER:08
drwxr-xr-x    - root  6 Jul 12:08  LNXPWRBN:00
drwxr-xr-x    - root  6 Jul 12:08  LNXSYBUS:00
drwxr-xr-x    - root  6 Jul 12:08  LNXSYBUS:01
.r--r--r-- 4.1k root  6 Jul 12:08  modalias
.r--r--r-- 4.1k root  6 Jul 12:08  path
drwxr-xr-x    - root  6 Jul 12:08  PNP0C14:03
drwxr-xr-x    - root  6 Jul 12:08  PNP0C14:04
drwxr-xr-x    - root  6 Jul 12:08  PNP0C14:05
drwxr-xr-x    - root  6 Jul 12:08  PNP0C14:06
drwxr-xr-x    - root  6 Jul 12:08  power
lrwxrwxrwx    - root  6 Jul 12:08  subsystem -> ../../bus/acpi
.rw-r--r-- 4.1k root  6 Jul 12:08  uevent
```
https://unix.stackexchange.com/questions/335822/how-can-i-get-more-information-about-acpi-devices

> Those are Plug and Play devices, AFAIK named after the legacy from the ISA bus era, but today identified by ACPI.

> PNP0c02 is a "motherboard resources" entry and INT3f0d seems to be a Intel Watchdog Timer.

```txt
🧀  ls -la /sys/bus/pnp/devices
lrwxrwxrwx - root 28 Jun 19:53  00:00 -> ../../../devices/pnp0/00:00
lrwxrwxrwx - root 28 Jun 19:53  00:01 -> ../../../devices/pnp0/00:01
lrwxrwxrwx - root 28 Jun 19:53  00:02 -> ../../../devices/pnp0/00:02
lrwxrwxrwx - root 28 Jun 19:53  00:03 -> ../../../devices/pnp0/00:03
lrwxrwxrwx - root 28 Jun 19:53  00:04 -> ../../../devices/pnp0/00:04
lrwxrwxrwx - root 28 Jun 19:53  00:05 -> ../../../devices/pnp0/00:05
lrwxrwxrwx - root 28 Jun 19:53  00:06 -> ../../../devices/pnp0/00:06
```
...
lrwxrwxrwx - root 19 Dec 10:58  r8169-0-500 -> ../../devices/pci0000:00/0000:00:1c.1/0000:05:00.0/mdio_bus/r8169-0-500
```

```txt
🧀  ls -la /sys/class/mdio_bus
lrwxrwxrwx - root 18 Dec 15:16  fixed-0 -> '../../devices/platform/Fixed MDIO bus.0/mdio_bus'/fixed-0
```
