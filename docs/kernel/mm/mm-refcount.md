...
2. 但是，page_mapcount 初始返回 0, 因为函数计算中加上了 1
3. 增加一个 mapcount 不会导致 refcount 增加 1 : 这是错误的，增加一个 mapcount 必然会增加一个 refcount
...
```
1. 因为 private 会增加 refcount
2. 为什么需要加上 1 ，我猜测是因为因为在 lru list 中导致的
