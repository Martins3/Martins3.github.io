...
compaction_suitable 中使用，该函数是分析一个 zone 的中内存
到底是不够还是碎片话过于严重。
...
1. 希望获取一个连续区域大小的 free page
2. 很多时候是为了分配大页，那么最后这个连续区域是一个大页的大小
3. 提前确定那些 page block 范围的页是
...
1. 前两个 bit 存储 `enum migratetype`
2. 第三个 bit 存储 `PB_migrate_skip`
