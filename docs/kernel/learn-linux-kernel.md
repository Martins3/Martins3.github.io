...
## 环境准备
<!-- 随着对于 Linux 的理解，愈发感觉到这个问题很重要 ，参考 tutorial 系列 -->
<!-- 1. 阅读环境准备 -->
...
### 感想
有的时候，感觉内核真的复杂，感觉都多了很长时间

### 从抽象到具体，在从具体到抽象

### 奇怪的感觉

之前看 intel sdm 3 中的虚拟化，完全不知道在说什么，但是
过了了两年，再看，似乎感觉每一句话都可以很容易在 kernel 中
找到对应的说明，好简单。

## 愈发的感觉操作系统，编译和 CPU 设计关系很大

阅读 perf book ，需要看多核 CPU 的设计，

__sync_add_and_fetch 这个东西也是 gcc 提供的。

操作系统，是如何最大化利用上硬件，如果不理解硬件，怎么可以?

编译，贴近 CPU 和 GPU ，对于微架构的需要理解。

1. 例如 bpftrace ，bcc 就是一个编译器和 linux kernel 的结合


不要限制自己的想象力，不要约束自己的可能性，将自己定义为问题的解决者，而不是简单的 Linux kernel 工程师。

## 不要去关心代码的调用的路径，而是关注数据是如何操作的
引用一下 perf book 中的 data owner 的思路看看
