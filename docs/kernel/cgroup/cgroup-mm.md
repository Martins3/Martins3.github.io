...
实际上，这才是服务器上大多数机器的状态。通过调用 try_to_free_mem_cgroup_pages 来实现 cgroup 中的内存回收。
...
page -> memcg -> parent memcg -> kernfs_node -> ino

## charge 和 uncharge 是关键
...
- mem_cgroup_uncharge_skmem 只是一个小小的封装而已

- mem_cgroup_uncharge_swap && mem_cgroup_try_charge_swap : only called by get_swap_page

```txt
@[
    mem_cgroup_uncharge_skmem+5
    __sk_mem_reduce_allocated+142
    tcp_recvmsg_locked+1273
    tcp_recvmsg+135
    inet_recvmsg+86
    sock_recvmsg+190
    sock_read_iter+147
...
root 中如果设置了限制，但是 A B 没有设置限制，A 申请，B 如何被影响的
```txt
root
/ \
A  B
...
哦，其实 4.18 只是统计不会限制，而 4.19 连统计都懒得搞了

[Why kmem should be disabled ?](https://www.spinics.net/lists/mm-commits/msg140715.html)

`__memcg_kmem_charge` : 最后调用到 `try_charge`
  - `__memcg_kmem_charge_page`
  - obj_cgroup_charge

的确 v2 是没有 kmem 限制的，从这里也可以看的很清楚:
https://www.alibabacloud.com/help/zh/alinux/support/differences-between-cgroup-v1-and-cgroup-v2#17831f21b2k3q


整个 charge 的过程为，但是从 __memcg_kmem_charge_page 无法找到
v2 不去 charge 的过程
```txt
@[
    __memcg_kmem_charge_page+5 // slub 分配空间的时候
    __alloc_frozen_pages_noprof+707
    alloc_pages_mpol+123
    alloc_pages_noprof+80
    get_free_pages_noprof+17
    __kvm_mmu_topup_memory_cache+146
    mmu_topup_memory_caches+62
    kvm_mmu_load+61
    vcpu_enter_guest.constprop.0+2870
...
@[
    __memcg_kmem_charge_page+5 // 其他的各种活动使用的内存
    __alloc_frozen_pages_noprof+707
    alloc_pages_mpol+123
    alloc_pages_noprof+80
    pte_alloc_one+29
    __pte_alloc+42
    do_anonymous_page+1312
    __handle_mm_fault+2944
    handle_mm_fault+383
...
这个 bactrace 是做什么的:
```txt
+ sudo bpftrace -e 'kprobe:kmem_cache_charge { @[kstack(bpftrace)] = count(); }
Attaching 2 probes...
^C
...
## cgroup memory
<!-- 89f1524a-2e61-452b-8487-8c5e94d068d2 -->

https://docs.kernel.org/admin-guide/cgroup-v2.html#memory-interface-files

也就是软硬 保护和限制，其他的都很容易了

|-------------|--------|-------------------------------------------------------------------|-------------------------------|
| memory.min  | 硬保护 | 保护内存不被回收，极端情况触发 OOM	保证核心服务的最低内存需求 | Guaranteed (请求值)           |
