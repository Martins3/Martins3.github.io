...
## suspend 和 dompmsuspend 区别

suspend 是在操作 qemu 进程，而 dompmsuspend 是在操作 guest 的虚拟机

- tools/virsh-domain.c : cmdSuspend --> qemuProcessStopCPUs

最后的 qmp 命令是 : stop 和 cont

## libvirt 是如何管理 cgroup 的

## qemu 的强制关机
- qemuDomainDestroyFlags
  - qemuProcessBeginStopJob
    - qemuProcessKill
      - virProcessKill : 直接 kill 一个 qemu 结束
    - virDomainObjBeginJob
      - virDomainObjBeginJobInternal
  - qemuProcessStop : 进行回收工作
    - qemuHostdevReAttachDomainDevices
      - qemuHostdevReAttachPCIDevices
        - virHostdevReAttachPCIDevices
          - virHostdevReAttachPCIDevicesImpl : 这里一共存在 5 个 步骤，应该就是需要的操作了
          - virHostdevDeleteMissingPCIDevices

echo 0000:3b:09.0 >  /sys/bus/pci/devices/0000:0b:09.0/driver/unbind

## console 和 ttyconsole 区别分析在 docs/kernel/tty 目录中

## 似乎也是可以 dump 的
 virsh domxml-to-native qemu-argv  --domain 264
