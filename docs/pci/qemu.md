...
```txt
(qemu) martins3:i440fx_init pci_type=i440FX pci-host-type=i440FX-pcihost
```
...
```c
[martins3:pci_data_read:127] 80000900
[martins3:pci_data_read:127] 80000a00
[martins3:pci_data_read:127] 80000b00
[martins3:pci_data_read:127] 80000c00
[martins3:pci_data_read:127] 80000d00
[martins3:pci_data_read:127] 80000e00
[martins3:pci_data_read:127] 80000f00
[martins3:pci_data_read:127] 80001000
[martins3:pci_data_read:127] 80009800
// ....
[martins3:pci_data_read:127] 8000a000
[martins3:pci_data_read:127] 8000a800
[martins3:pci_data_read:127] 8000b000
[martins3:pci_data_read:127] 8000b800
[martins3:pci_data_read:127] 8000c000
[martins3:pci_data_read:127] 8000c800
[martins3:pci_data_read:127] 8000d000
[martins3:pci_data_read:127] 8000d800
[martins3:pci_data_read:127] 8000e000
[martins3:pci_data_read:127] 8000e800
[martins3:pci_data_read:127] 8000f000
[martins3:pci_data_read:127] 8000f800
```
...
[martins3:pci_update_mappings:1354] ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
...
[martins3:pci_update_mappings:1357] c020 -> ffffffffffffffff
PCI: init bdf=00:01.3 id=8086:7113
Using pmtimer, ioport 0x608
PCI: init bdf=00:02.0 id=1234:1111
[martins3:pci_update_mappings:1357] fd000000 -> ffffffffffffffff
[martins3:pci_update_mappings:1357] febd4000 -> ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
PCI: init bdf=00:03.0 id=8086:10d3
[martins3:pci_update_mappings:1357] feb80000 -> ffffffffffffffff
[martins3:pci_update_mappings:1357] feba0000 -> ffffffffffffffff
[martins3:pci_update_mappings:1357] c000 -> ffffffffffffffff
[martins3:pci_update_mappings:1357] febd0000 -> ffffffffffffffff
[martins3:pci_update_mappings:1354] ffffffffffffffff
```
...
## cmask wmask 和 w1cmask

- do_pci_register_device
  - pci_config_alloc : 分配空间
  - [ ] pci_init_cmask
  - [ ] pci_init_wmask
  - [ ] pci_init_w1cmask ????

使用位置:
1. pci_default_write_config
```c
    for (i = 0; i < l; val >>= 8, ++i) {
...
```
  - [ ] wmask 似乎如果一开始就一个初始数值，是永远都无法触及的

2. pci_init_mask_bridge ??
```c
...
```
3. pcie_add_capability : 让 capability 是只读的

- get_pci_config_device 中只是使用的这个, load 指的是 vmstate 的 load 的过程.

- [ ] 为什么不对称:
  - pci_add_capability : 没有处理 w1cmask
  - pcie_add_capability : 处理 w1cmask

 - pcie_cap_deverr_init

- 大多数 wmask 和 w1cmask 都是同时出现的，但是在 pci_register_bar 中，通过 wmask 来描述大小 k

cmask 到处的都是赋值，只有在 get_pci_config_device 的时候用到，其左右应该就是在此处！

```c
        if ((config[i] ^ s->config[i]) &
            s->cmask[i] & ~s->wmask[i] & ~s->w1cmask[i]) {
```
1. cmask 如果检查，一般是 0xFF，表示都检查
2. s->w1cmask[i] 一般是 0
3. ~s->wmask[i] 一般是 check 的

```diff
History:        #0
Commit:         bd4b65ee5e5f750da709ac10c70266876e515c23
Author:         Michael S. Tsirkin <mst@redhat.com>
Committer:      Anthony Liguori <anthony@codemonkey.ws>
Author Date:    Mon 22 Jun 2009 12:49:40 AM CST
Committer Date: Wed 24 Jun 2009 10:09:14 PM CST
...
[martins3:pci_update_mappings:1361] ffffffffffffffff -> c020
martins3
martins3
[martins3:pci_update_mappings:1361] c020 -> ffffffffffffffff
[martins3:pci_update_mappings:1361] ffffffffffffffff -> c020
martins3
martins3
[martins3:pci_update_mappings:1361] c020 -> ffffffffffffffff
