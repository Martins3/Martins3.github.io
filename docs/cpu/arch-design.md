...
- 如何实现 CPU 虚拟化 : 导致 vmexit 的指令是 特权指令 的子集吗? 如果不是，实现一些虚拟化产品会有难度。
- 内存虚拟化 : intel vt-x
- 如何支持嵌套虚拟化
- 定义 CPU model ，让虚拟机可以从老的 CPU 热迁移到新的 CPU 中。
...
- 在 page table 上存在 flag 来支持 idle page tracking

### 兼容性
如何保持软件接口的兼容性，但是不会拖累新处理器的处理。
例如 x86 来保持指令的兼容
