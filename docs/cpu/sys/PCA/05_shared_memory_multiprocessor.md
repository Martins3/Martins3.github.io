...
2. **Shared** means the block is present in unmodified state in this cache, main memory is up-to-date,
   and _zero or more_ other caches may also have an up-to-date (shared) copy.
...
4. The other new concept needed to support write-back protocols
   ...
   bus transaction and “**flush**” the contents of the referenced block onto the _bus_, rather than allow
...
By adding a state that indicates that the block is _the only (exclusive)
copy but is not modified_ and loading the block in this state, we can save the latter transaction
...
modified (i.e., the main memory is up-to-date). S means that potentially _two or more_ processors
